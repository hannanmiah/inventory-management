      ...
      <template #content>
        <Form class="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4" @submit="save">
          ...
          <div class="flex flex-col">
            ...
            <span class="text-red-500" v-if="form.errors.contact_info?.phone">{{ form.errors.contact_info?.phone }}</span>
          </div>
          <div class="flex flex-col">
            <label for="email">Email</label>
            <input-text v-model="form.contact_info.email" type="email" id="email" name="email"/>
            <span class="text-red-500" v-if="form.errors.contact_info?.email">{{ form.errors.contact_info?.email }}</span>
          </div>
