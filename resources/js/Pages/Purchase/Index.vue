...
const {purchases, suppliers} = defineProps({
  purchases: {
    type: Object,
    required: true,
  },
    ...
    <Card>
      <template #title>
        <section class="flex space-x-2 md:space-x-4">
          ...
          <InputText v-model="filter.total_amount" type="number" placeholder="Filter with amount"/>
          <Select v-model="filter.supplier_id" option-value="id" option-label="name" :options="suppliers"
                  placeholder="Filter By Supplier"/>
        </section>
      </template>
      <template #content>
        <DataTable :value="purchases.data" paginator :rows="10" :rowsPerPageOptions="[5, 10, 20, 50]">
          <template #header>
            <section class="flex justify-between items-center">
              <h1 class="text-lg md:text-2xl font-medium md:font-bold">Purchases</h1>
              <Button as="Link" label="New" :href="route('purchases.create')" icon="pi pi-plus" class="mr-2"/>
            </section>
          </template>
          ...
          <Column field="purchase_items" header="Items">
            <template #body="slotProps">
              <ul class="inline-flex flex-col space-y-1 md:space-y-2">
                <li v-for="item in slotProps.data.purchase_items" :key="item.id">
          ...
          <Column header="Actions">
            <template #body="slotProps">
              <section class="inline-flex space-x-1">
                <Button as="Link" :href="route('purchases.show',slotProps.data.id)" severity="success"
                        variant="outlined" icon="pi pi-eye" rounded aria-label="View"/>
                <Button as="Link" :href="route('purchases.edit',slotProps.data.id)" severity="info"
      ...
      <template #footer> In total there are {{ purchases.data.length }} purchases.</template>
    </Card>
  </DefaultLayout>
</template>
